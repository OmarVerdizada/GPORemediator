(() => {
  ...
  function render(){
    ...
    panel.innerHTML=`<div class="gr-auto-head"><div><h2>GPO · Sazlamalar</h2><p>Avtomatik aşkarlama · Əl ilə düzəliş mümkündür</p></div><button data-close class="gr-auto-close" aria-label="Bağla">×</button></div><div class="gr-auto-body" aria-busy="${state.busy}">
      ...
      <div class="gr-auto-note">${state.session?.mode==='WINDOWS'?'Windows rejimi · REAL':'Setup rejimi · yalnız konfiqurasiya'}. GPO və link hədəfləri qoşulduqdan sonra siyahıdan seçilir. GPO GUID-ni əl ilə daxil etmək lazım deyil.</div>
      ${warnings.length?`<div class="gr-auto-note warn">${warnings.map(esc).join('<br>')}</div>`:''}
      <form id="windows-setup" class="gr-auto-card"><h3>Windows / AD bağlantısı</h3><p>Domen, DC və operator ilk açılışda aşkarlanır. Saxlanmış və əl ilə yazılmış məlumatlar avtomatik əvəz olunmur.</p><div class="gr-auto-grid">
      ${field('domain','AD domain','example.com')}${field('domainController','Writable domain controller','dc01.example.com')}
      ${field('backupPath','Backup path','C:\\ProgramData\\GpoRemediator\\Backups')}
      ...
      <p>Tool yalnız bu serverdə <strong>127.0.0.1</strong> üzərindən işləyir. Sertifikat tələb olunmur və şəbəkədən giriş qəbul edilmir. Saxlama yazma icazəsini bağlı saxlayır.</p>
      ...
      <div class="gr-auto-card"><h3>Hazırlıq və yazma icazəsi</h3>${state.session?.mode==='WINDOWS'?'<button class="gr-auto-btn" data-readiness>Real AD hazırlığını yoxla</button>':'<p>Setup rejimində real AD readiness icra edilmir. Sazlamaları saxlayıb Windows rejiminə keçin.</p>'}
  ...
  function init(){
    ...
    panel.addEventListener('submit',e=>{e.preventDefault();capture();const c={...draft};action(async()=>{
      const payload={...c,urls:location.origin,approvedGpoIds:[],authorizedOus:[],allowedHosts:[],allowedOperators:String(c.allowedOperators||'').split(/[\n;]/).map(v=>v.trim()).filter(Boolean),autoRestart:true};
      const result=await api('/setup/config',payload);state.notice='Saxlanıldı. Windows rejimi yenidən başladılır; yazma bağlıdır.';
      if(result.restartScheduled)setTimeout(()=>{location.href=location.origin+'/#/settings';},3200);
    });});
